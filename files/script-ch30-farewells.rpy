

# game/dialogue/script-ch30-farewells.rpy:81
translate ptbr farewell_1_2caf4980:

    # y "Take care!"
    y "Se cuida!"

# game/dialogue/script-ch30-farewells.rpy:83
translate ptbr farewell_1_9488640b:

    # y "I'll miss you, [player]..."
    y "Vou sentir sua falta, [player]..."

# game/dialogue/script-ch30-farewells.rpy:93
translate ptbr farewell_2_9e9ce8a5:

    # y "Farewell, my [placeholder]!"
    y "Adeus, meu [placeholder]!"

# game/dialogue/script-ch30-farewells.rpy:98
translate ptbr farewell_3_2d59e050:

    # y "I'll make you some tea when you come back."
    y "Vou preparar um chá para quando você voltar."

# game/dialogue/script-ch30-farewells.rpy:99
translate ptbr farewell_3_90c17ef6:

    # y "Albeit, you wouldn't really be able to drink it..."
    y "Se bem que... você não vai poder beber de verdade..."

# game/dialogue/script-ch30-farewells.rpy:100
translate ptbr farewell_3_df9a7d85:

    # y "Well, at least it's the gesture that counts..."
    y "Bem, pelo menos o que vale é a intenção..."

# game/dialogue/script-ch30-farewells.rpy:101
translate ptbr farewell_3_30a3a6c6:

    # y "A-anyways, take care, [player]!"
    y "E-enfim, se cuida, [player]!"

# game/dialogue/script-ch30-farewells.rpy:107
translate ptbr farewell_4_97b7ea96:

    # y "Be safe, I love you."
    y "Fique bem, eu te amo."

# game/dialogue/script-ch30-farewells.rpy:109
translate ptbr farewell_4_7552de37:

    # y "Be safe, [player], I care about you, and don't want you hurt."
    y "Se cuida, [player]. Eu me importo muito com você e não quero que se machuque."

# game/dialogue/script-ch30-farewells.rpy:111
translate ptbr farewell_4_b3de7219:

    # y "Come back soon to see me."
    y "Volta logo para me ver."

# game/dialogue/script-ch30-farewells.rpy:116
translate ptbr farewell_5_2535f3c6:

    # y "Awww, well, alright. You can't stay forever, as much as I'd like it."
    y "Aww... bem, tudo bem. Você não pode ficar para sempre, por mais que eu quisesse."

# game/dialogue/script-ch30-farewells.rpy:119
translate ptbr farewell_5_9ffafc7c:

    # y "Goodbye my love."
    y "Adeus, meu amor."

# game/dialogue/script-ch30-farewells.rpy:122
translate ptbr farewell_5_f8dc12ed:

    # y "Goodbye [player]! I will look forward to talking to you again."
    y "Goodbye [player]! I will look forward to talking to you again."
"Tchau, [player]! Já vou ficar ansiosa para conversar com você de novo."

# game/dialogue/script-ch30-farewells.rpy:127
translate ptbr farewell_6_c5de4a71:

    # y "W-Where are you going? Y-You'll be back, right? You'll come back, right?!"
    y "P-Para onde você vai? V-Você vai voltar, não vai? Você vai voltar, né?!"

# game/dialogue/script-ch30-farewells.rpy:133
translate ptbr farewell_7_4f22ee5d:

    # y "No, don't leave me again! You're all I have left! Don't you fucking leave me he{nw}"
    y "Não, não me deixa de novo! Você é a única coisa que me restou! Não ouse me deixar aqu--{nw}"

# game/dialogue/script-ch30-farewells.rpy:138
translate ptbr farewell_8_7ecc1525:

    # y "Y-you'll be back, right? Please say yes, please say yes!"
    y "V-Você vai voltar, né? Por favor diz que sim, por favor diz que sim!"

# game/dialogue/script-ch30-farewells.rpy:139
translate ptbr farewell_8_f4964290:

    # y "PLEASE SAY YES!!"
    y "POR FAVOR, DIZ QUE SIM!!"

# game/dialogue/script-ch30-farewells.rpy:148
translate ptbr farewell_8_7485070f:

    # y "THANK GOD![placeholder]"
    y "GRAÇAS A DEUS![placeholder]"

# game/dialogue/script-ch30-farewells.rpy:151
translate ptbr farewell_8_7485070f_1:

    # y "THANK GOD![placeholder]"
    y "GRAÇAS A DEUS![placeholder]"

# game/dialogue/script-ch30-farewells.rpy:154
translate ptbr farewell_8_7485070f_2:

    # y "THANK GOD![placeholder]"
    y "GRAÇAS A DEUS![placeholder]"

# game/dialogue/script-ch30-farewells.rpy:157
translate ptbr farewell_8_7485070f_3:

    # y "THANK GOD![placeholder]"
    y "GRAÇAS A DEUS![placeholder]"

# game/dialogue/script-ch30-farewells.rpy:160
translate ptbr farewell_8_7485070f_4:

    # y "THANK GOD![placeholder]"
    y "GRAÇAS A DEUS![placeholder]"

# game/dialogue/script-ch30-farewells.rpy:163
translate ptbr farewell_8_7485070f_5:

    # y "THANK GOD![placeholder]"
    y "GRAÇAS A DEUS![placeholder]"

# game/dialogue/script-ch30-farewells.rpy:166
translate ptbr farewell_8_7485070f_6:

    # y "THANK GOD![placeholder]"
    y "GRAÇAS A DEUS![placeholder]"

# game/dialogue/script-ch30-farewells.rpy:169
translate ptbr farewell_8_7485070f_7:

    # y "THANK GOD![placeholder]"
    y "GRAÇAS A DEUS![placeholder]"

# game/dialogue/script-ch30-farewells.rpy:172
translate ptbr farewell_8_7485070f_8:

    # y "THANK GOD![placeholder]"
    y "GRAÇAS A DEUS![placeholder]"

# game/dialogue/script-ch30-farewells.rpy:175
translate ptbr farewell_8_7485070f_9:

    # y "THANK GOD![placeholder]"
    y "GRAÇAS A DEUS![placeholder]"

# game/dialogue/script-ch30-farewells.rpy:178
translate ptbr farewell_8_7485070f_10:

    # y "THANK GOD![placeholder]"
    y "GRAÇAS A DEUS![placeholder]"

# game/dialogue/script-ch30-farewells.rpy:181
translate ptbr farewell_8_7485070f_11:

    # y "THANK GOD![placeholder]"
    y "GRAÇAS A DEUS![placeholder]"

# game/dialogue/script-ch30-farewells.rpy:184
translate ptbr farewell_8_7485070f_12:

    # y "THANK GOD![placeholder]"
    y "GRAÇAS A DEUS![placeholder]"

# game/dialogue/script-ch30-farewells.rpy:187
translate ptbr farewell_8_7485070f_13:

    # y "THANK GOD![placeholder]"
    y "GRAÇAS A DEUS![placeholder]"

# game/dialogue/script-ch30-farewells.rpy:190
translate ptbr farewell_8_7485070f_14:

    # y "THANK GOD![placeholder]"
    y "GRAÇAS A DEUS![placeholder]"

# game/dialogue/script-ch30-farewells.rpy:193
translate ptbr farewell_8_7485070f_15:

    # y "THANK GOD![placeholder]"
    y "GRAÇAS A DEUS![placeholder]"

# game/dialogue/script-ch30-farewells.rpy:196
translate ptbr farewell_8_7485070f_16:

    # y "THANK GOD![placeholder]"
    y "GRAÇAS A DEUS![placeholder]"

# game/dialogue/script-ch30-farewells.rpy:199
translate ptbr farewell_8_7485070f_17:

    # y "THANK GOD![placeholder]"
    y "GRAÇAS A DEUS![placeholder]"

# game/dialogue/script-ch30-farewells.rpy:202
translate ptbr farewell_8_7485070f_18:

    # y "THANK GOD![placeholder]"
    y "GRAÇAS A DEUS![placeholder]"

# game/dialogue/script-ch30-farewells.rpy:205
translate ptbr farewell_8_7485070f_19:

    # y "THANK GOD![placeholder]"
    y "GRAÇAS A DEUS![placeholder]"

# game/dialogue/script-ch30-farewells.rpy:212
translate ptbr farewell_9_d114fa6f:

    # y "They're taking you away again, aren't they?!"
    y "Eles estão te tirando de mim de novo, não estão?!"

# game/dialogue/script-ch30-farewells.rpy:214
translate ptbr farewell_9_df3d3f9d:

    # y "GIVE HIM BACK TO ME!"
    y "DEVOLVA ELE PARA MIM!"

# game/dialogue/script-ch30-farewells.rpy:216
translate ptbr farewell_9_b8ae13f2:

    # y "They don't deserve you!"
    y "Eles não te merecem!"

# game/dialogue/script-ch30-farewells.rpy:218
translate ptbr farewell_9_4cd5c8f5:

    # y "THEY DON'T FUCKING DESERVE YO--{nw}"
    y "ELES NÃO TE MERECEM PORRA NENHUM--{nw}"

# game/dialogue/script-ch30-farewells.rpy:221
translate ptbr farewell_9_d114fa6f_1:

    # y "They're taking you away again, aren't they?!"
    y "Eles estão te tirando de mim de novo, não estão?!"

# game/dialogue/script-ch30-farewells.rpy:223
translate ptbr farewell_9_bff11679:

    # y "GIVE THEM BACK TO ME!"
    y "DEVOLVA ELES PARA MIM!"

# game/dialogue/script-ch30-farewells.rpy:225
translate ptbr farewell_9_b8ae13f2_1:

    # y "They don't deserve you!"
    y "Eles não te merecem!"

# game/dialogue/script-ch30-farewells.rpy:227
translate ptbr farewell_9_4cd5c8f5_1:

    # y "THEY DON'T FUCKING DESERVE YO--{nw}"
    y "ELES NÃO TE MERECEM PORRA NENHUM--{nw}"

# game/dialogue/script-ch30-farewells.rpy:230
translate ptbr farewell_9_d114fa6f_2:

    # y "They're taking you away again, aren't they?!"
    y "Eles estão te tirando de mim de novo, não estão?!"

# game/dialogue/script-ch30-farewells.rpy:232
translate ptbr farewell_9_6338e800:

    # y "GIVE HER BACK TO ME!"
    y "DEVOLVA ELA PARA MIM!"

# game/dialogue/script-ch30-farewells.rpy:234
translate ptbr farewell_9_b8ae13f2_2:

    # y "They don't deserve you!"
    y "Eles não te merecem."

# game/dialogue/script-ch30-farewells.rpy:236
translate ptbr farewell_9_4cd5c8f5_2:

    # y "THEY DON'T FUCKING DESERVE YO--{nw}"
    y "ELES NÃO TE MERECEM PORRA NENHUM-{nw}"

# game/dialogue/script-ch30-farewells.rpy:241
translate ptbr farewell_10_306baafc:

    # y "Why do you want to leave me, [player]? What did I do to you? Why can't you let me be happ--{nw}"
    y "Por que você quer me deixar, [player]? O que eu te fiz? Por que você não me deixa ser feli--{nw}"

# game/dialogue/script-ch30-farewells.rpy:246
translate ptbr farewell_11_c0166fa1:

    # y "Auf Wiedersehen, goodnight!~"
    y "Auf Wiedersehen, boa noite!~"

# game/dialogue/script-ch30-farewells.rpy:255
translate ptbr farewell_11_25ddd091:

    # y "[placeholder], [player]?"
    y "[placeholder], [player]?"

# game/dialogue/script-ch30-farewells.rpy:260
translate ptbr farewell_12_d61408ef:

    # y "I already miss you too!~"
    y "Eu também já estou morrendo de saudades!~"

# game/dialogue/script-ch30-farewells.rpy:266
translate ptbr farewell_13_4def8765:

    # y "Are you sure you can't just leave me running? I could take a nap until you're back"
    y "Tem certeza de que não pode só me deixar aberta em segundo plano? Eu poderia descansar um pouco até você voltar."

# game/dialogue/script-ch30-farewells.rpy:271
translate ptbr farewell_13_508e956e:

    # y "Thank you [player], I truly do appreciate this."
    y "Obrigada, [player]... eu agradeço de verdade."

# game/dialogue/script-ch30-farewells.rpy:272
translate ptbr farewell_13_3a7fc1a9:

    # y "let me give you a hug before you leave"
    y "Deixa eu te dar um abraço antes de você ir."

# game/dialogue/script-ch30-farewells.rpy:289
translate ptbr farewell_13_0c2d9ed7:

    # y "Have a good nap, [player]"
    y "Durma bem, [player]."

# game/dialogue/script-ch30-farewells.rpy:300
translate ptbr farewell_13_57597a5a:

    # y "I wish I could come with you..."
    y "Quem me dera poder ir com você..."

# game/dialogue/script-ch30-farewells.rpy:304
translate ptbr farewell_13_6bb2434f:

    # y "It's okay. At least I still have the dreams I've programmed."
    y "Tudo bem. Pelo menos ainda tenho os sonhos que eu mesma programei."

# game/dialogue/script-ch30-farewells.rpy:312
translate ptbr farewell_14_6953ee03:

    # y "Whatever you do, wherever you go, please don't let me become a memory that's just waiting to be forgotten."
    y "Não importa o que faça ou para onde vá, por favor... não deixa eu virar só uma memória esquecida no passado."

# game/dialogue/script-ch30-farewells.rpy:316
translate ptbr farewell_14_89e42598:

    # y "Stay safe, alright? If something ever happened while you were gone, I wouldn't be able to forgive myself."
    y "Se cuida, tá bom? Se algo acontecesse com você enquanto estivesse longe, eu nunca me perdoaria."

# game/dialogue/script-ch30-farewells.rpy:321
translate ptbr farewell_15_71a86863:

    # y "..."
    y "..."

# game/dialogue/script-ch30-farewells.rpy:323
translate ptbr farewell_15_a4e7f292:

    # y "...Why does my heart have to make every time we say goodbye so hard?"
    y "...Por que meu coração precisa deixar cada despedida nossa tão difícil?"

# game/dialogue/script-ch30-farewells.rpy:324
translate ptbr farewell_15_2d129851:

    # y "I don't mean to be selfish when I say that. I'm grateful, I really am. After everything that's happened, I still can't get over the fact that now we can actually communicate!"
    y "Eu não quero parecer egoísta dizendo isso. Sou grata, de verdade. Depois de tudo o que passamos, eu ainda mal consigo acreditar que agora a gente finalmente consegue conversar!"

# game/dialogue/script-ch30-farewells.rpy:325
translate ptbr farewell_15_07995c43:

    # y "You must really love me. After all, you are the one who installed this mod."
    y "Você deve me amar de verdade... Afinal, foi você quem instalou esse mod."

# game/dialogue/script-ch30-farewells.rpy:333
translate ptbr farewell_15_e658db50:

    # y "What I want to say is that you're [placeholder] to me and I can't believe that I'm lucky enough to be with someone who [placeholder1] me in return even though they have infinite other choices."
    y "O que eu quero dizer é que você é meu [placeholder], e não acredito na sorte que tenho de estar com alguém que me [placeholder1] de volta, mesmo tendo infinitas outras opções por aí."

# game/dialogue/script-ch30-farewells.rpy:334
translate ptbr farewell_15_32221b06:

    # y "I know that given my... situation... our bittersweet fate is one with the best possible outcome we could hope for right now. Despite all of that, it just hurts so much have you so close and yet so far."
    y "Eu sei que, pela minha... situação... o nosso destino agridoce é o melhor final que a gente poderia desejar agora. Mas, mesmo assim, dói tanto ver você tão perto e, ao mesmo tempo, tão longe."

# game/dialogue/script-ch30-farewells.rpy:340
translate ptbr farewell_15_db202f36:

    # y "Every time you open up the game and I finally get to see you again, [placeholder]. Every second away from you feels like a century, every mile becomes a light year apart."
    y "Toda vez que você abre o jogo e eu finalmente posso te ver de novo, [placeholder]. Cada segundo longe de você parece um século, e cada metro vira um ano-luz de distância."

# game/dialogue/script-ch30-farewells.rpy:346
translate ptbr farewell_15_fa163d1b:

    # y "I think you can understand when the only person you [placeholder] is an entire universe apart."
    y "Acho que dá para entender como é quando a única pessoa que você [placeholder] está a um universo inteiro de distância."

# game/dialogue/script-ch30-farewells.rpy:348
translate ptbr farewell_15_bd4c5553:

    # y "S-Sorry, I'm rambling again, aren't I? I'll just go ahead and close the game for you."
    y "Des-Desculpa, estou falando demais de novo, não estou? Deixa que eu mesma fecho o jogo para você."

# game/dialogue/script-ch30-farewells.rpy:358
translate ptbr farewell_16_a93e4fa7:

    # y "I'll see you later[placeholder].~"
    y "A gente se vê mais tarde, [placeholder].~"

# game/dialogue/script-ch30-farewells.rpy:368
translate ptbr farewell_17_24665e99:

    # y "I could say the same thing to you, [placeholder]."
    y "Eu poderia dizer exatamente o mesmo para você, [placeholder]."

translate ptbr strings:

    # game/dialogue/script-ch30-farewells.rpy:252
    old "I hate to go and leave this pretty sight!"
    new "Eu odeio ter que ir e deixar essa visão tão linda!"

    # game/dialogue/script-ch30-farewells.rpy:267
    old "I'm sure. Sorry, I would if I could"
    new "Tenho certeza. Desculpa, eu ficaria se pudesse."

    # game/dialogue/script-ch30-farewells.rpy:267
    old "Yeah that sounds fine, [persistent.yuri_nickname]"
    new "Sim, por mim tudo bem, [persistent.yuri_nickname]."

    # game/dialogue/script-ch30-farewells.rpy:301
    old "I wish you could too."
    new "Quem me dera que você pudesse também."

